...
# ITA-OS: Include additional PA packages if the file exists
if [[ -f "$build_cache_dir/airootfs/root/omarchy/install/ita-os.packages" ]]; then
  echo "🇮🇹 ITA-OS: Adding Italian PA packages..."
  all_packages+=($(grep -v '^#' "$build_cache_dir/airootfs/root/omarchy/install/ita-os.packages" | grep -v '^$'))
fi
